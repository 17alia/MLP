...
def ReadNormalizedOptdigitsDataset(training_filename, validation_filename, test_filename):
    ...
    X_trn = np.delete(X_trn, 56, axis=1)
    X_trn = np.delete(X_trn, 39, axis=1)
    X_trn = np.delete(X_trn, 0, axis=1)
    X_val = np.delete(X_val, 56, axis=1)
    X_val = np.delete(X_val, 39, axis=1)
    X_val = np.delete(X_val, 0, axis=1)
    X_tst = np.delete(X_tst, 56, axis=1)
    X_tst = np.delete(X_tst, 39, axis=1)
    X_tst = np.delete(X_tst, 0, axis=1)
